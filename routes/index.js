...
router.get('/data', function(req, res) {
    ...
    var tourism_query = "SELECT row_to_json(fc) FROM ( SELECT 'FeatureCollection' As type, array_to_json(array_agg(f) LIMIT 1) As features FROM (SELECT 'Feature' As type, ST_AsGeoJSON(lg.geom)::json As geometry, row_to_json(row(osm_id, (SELECT CAST(AVG(rate) AS INT) FROM ratings_tourism As Rat WHERE Rat.tourism_id = lg.osm_id LIMIT 1), name, tourism, (SELECT CASE WHEN session_id = '" + req.sessionID + "' THEN TRUE ELSE FALSE END FROM ratings_tourism As Sess WHERE Sess.tourism_id = lg.osm_id LIMIT 1)) LIMIT 1) As properties FROM tourism As lg WHERE lg.tourism IN ('artwork', 'attraction', 'picnic_site', 'theme_park', 'viewpoint', 'yes', 'hunting_lodge', 'gallery', 'zoo', 'wilderness_hut', 'museum') ORDER BY ST_Distance(lg.geom, ST_GeomFromText('POINT(" + req.query.long + " " + req.query.lat + ")',4326)) ASC LIMIT 1500) As f) As fc";

    console.log(tourism_query)
